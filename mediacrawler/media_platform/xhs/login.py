...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_mobile(self):
        ...
        # Check if the number is a mainland China number (starts with 1 and is 11 digits, or starts with 86)
        is_china_number = (len(self.login_phone) == 11 and self.login_phone.startswith("1")) or self.login_phone.startswith("86") or self.login_phone.startswith("+86")
        ...
        if is_china_number:
            ...
            # Wait for SMS code from cache (Redis/Memory)
        ...
        if not sms_code_value:
            # Manual mode for international numbers or when cache failed
            print("\n" + "="*70)
            print("PANDUAN LOGIN NOMOR TELEPON:")
            print("1. Pada browser yang terbuka, silakan pilih kode negara (+62 untuk Indonesia).")
            print(f"2. Masukkan nomor HP Anda: {self.login_phone}")
            print("3. Klik tombol 'Kirim Kode Verifikasi' (Send Code) di browser.")
            print("="*70 + "\n")
            
            try:
                sms_code_val = await asyncio.to_thread(input, ">> Masukkan 6 Digit Kode SMS Verifikasi yang Anda terima: ")
                sms_code_value = sms_code_val.strip().encode()
            except Exception as e:
                utils.logger.error(f"Error reading terminal input: {e}")
                sms_code_value = b""
